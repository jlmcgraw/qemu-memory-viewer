...
try:  # pragma: no cover - exercised when numpy is installed
    ...
    def _normalize_shape(
        shape: int | Sequence[int] | tuple[Sequence[int], ...],
    ) -> tuple[int, ...]:
    ...
    def _result_shape_static(
        shape: Sequence[int], indices: tuple[object, ...]
    ) -> tuple[int, ...]:
        if not indices:
            return tuple(shape)
        if not shape:
    ...
    def _extract_flat_static(
        flat: Sequence[int], shape: Sequence[int], indices: tuple[object, ...]
    ) -> list[int]:
        if not indices:
            return list(flat)
        if not shape:
    ...
    class ndarray:
        ...
        def __iter__(self) -> Iterator[int | ndarray]:
            if len(self.shape) == 1:
    ...
    def zeros(shape: int | Sequence[int], dtype: _UInt8DType = uint8) -> ndarray:
    ...
    def _infer_shape(obj: Sequence[object] | object) -> tuple[int, ...]:
    ...
    def _flatten(obj: Sequence[object] | object) -> Iterator[Number]:
        if isinstance(obj, (list, tuple)):
            for item in obj:
    ...
    class _CompatNDArray(_np.ndarray):
        ...
        def __array_finalize__(
            self, obj: object
        ) -> None:  # pragma: no cover - numpy protocol hook
            return None

        def tolist(
            self,
        ) -> list[int]:  # pragma: no cover - behaviour verified via tests
    ...
    def zeros(shape: int | Sequence[int], dtype: object = uint8) -> _CompatNDArray:
