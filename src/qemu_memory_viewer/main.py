...
# /// script
"""Interactive viewer for live QEMU memory state."""
...
def clamp(v: int, lo: int, hi: int) -> int:
    """Return ``v`` constrained to the inclusive range ``[lo, hi]``."""
    return lo if v < lo else hi if v > hi else v


def pick_mono_font(size: int = 13) -> ImageFont.FreeTypeFont:
    """Return a monospace font object for rendering debug panels."""
    try:
        from matplotlib import font_manager as fm
        from PIL import ImageFont
    except ModuleNotFoundError as exc:  # pragma: no cover
        # Exercised when optional dependencies are missing.
...
def bytes_to_cp437_lines(block: np.ndarray) -> list[str]:
    """Convert a block of bytes into printable CP437 text lines."""
    lines: list[str] = []
...
def render_text_panel(block: np.ndarray, font: ImageFont.FreeTypeFont) -> np.ndarray:
    """Render a magnified CP437 text panel for the selected bytes."""
    try:
        from PIL import Image, ImageDraw
    except ModuleNotFoundError as exc:  # pragma: no cover
        # Exercised when Pillow is missing.
...
class QMP:
    """Tiny helper for communicating with QEMU's QMP socket."""

    def __init__(self, path: str) -> None:
        """Initialize the helper with the UNIX-domain socket ``path``."""
        self.path = path
        self.sock: socket.socket | None = None
        self.buf = b""

    def connect(self) -> None:
        """Open the socket connection and negotiate capabilities."""
    ...
    def _send_json(self, obj: dict[str, object]) -> None:
        """Send a single JSON object over the QMP socket."""
    ...
    def _recv_json(self) -> dict[str, object]:
        """Receive the next JSON object from the QMP socket."""
        if self.sock is None:
            msg = "QMP socket has not been connected"
            raise RuntimeError(msg)
    ...
    def hmp(self, cmd: str) -> str:
        """Execute a human monitor command and return its textual output."""
        self._send_json(
            {
...
class MemoryMapping:
    ...
    @property
    def size(self) -> int:
        """Return the length of the mapping in bytes."""
...
def parse_register_dump(text: str) -> dict[str, int]:
    """Parse a human monitor register dump into a mapping of register values."""
...
def parse_memory_mappings(text: str) -> list[MemoryMapping]:
    """Extract memory mappings from a QEMU ``info mtree`` output block."""
...
def build_mapping_mask(
    ...
    """Return a mask describing which bytes fall inside ``mappings``."""
...
def compute_pointer_address(
    ...
    """Calculate a linear address from ``registers`` according to ``spec``."""
...
def qmp_read_bytes(
    q: QMP, start: int, count: int, chunk_size: int = MAPPING_FETCH_CHUNK
) -> np.ndarray:
...
def build_mapping_overlay_data(
    ...
    for mapping in mappings:
        ...
        try:
            ...
            logger.debug(
                "Unable to read mapping bytes for 0x%X-0x%X", start, end, exc_info=True
            )
            continue
...
def apply_overlay_block(
    ...
    def mask_any(obj: object) -> bool:
        try:
            ...
            return bool(obj)
        return any(
            mask_any(obj[idx])  # type: ignore[index]
...
def render_vga_text(
    ...
    """Render the VGA character buffer to an RGB image array."""
    try:
        from PIL import Image, ImageDraw
    except ModuleNotFoundError as exc:  # pragma: no cover
        # Exercised when Pillow is missing.
...
def main() -> None:
    """Entry point for the interactive QEMU memory viewer application."""
    try:
    ...
    p.add_argument(
        ...
        type=int,
        default=1024,
        help="bytes per row in RAM view",
    )
    p.add_argument(
        "--height",
        type=int,
        default=1024,
        help="rows in RAM view",
    )
    ...
    if overlay_mappings:
        ...
        if mapping_visible:
            ...
            for idx, mapping in enumerate(mapping_visible, start=1):
                ...
                overlay_label = (
                    f"{mapping.label} (0x{mapping.start:08X}-0x{mapping.end:08X})"
                )
    ...
    try:
        ...
        logger.debug("Unable to fetch register dump", exc_info=True)
    ...
    def update(_: object) -> tuple[object, ...]:
        ...
        cx = clamp(
            sel_cx if sel_cx is not None else args.width // 2,
            0,
            args.width - 1,
        )
        cy = clamp(
            sel_cy if sel_cy is not None else args.height // 2,
            0,
            args.height - 1,
        )
        ...
        formatted_addr = f"0x{addr:08X} ({format_kib_from_bytes(addr)})"
        if region is None:
            hover_lines = ["Unlabeled memory", formatted_addr]
        else:
            hover_lines = [region.label, formatted_addr]
        hover_text.set_text("\n".join(hover_lines))
        ...
        try:
            ...
            logger.debug("Unable to refresh VGA panel", exc_info=True)
